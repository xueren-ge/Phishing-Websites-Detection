...
class FeatureExtraction:
    ...
    def havingIP(self, url):
        ...
        match = re.search('(([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d'
                          '|25[0-5])\\.([01]?\\d\\d?|2[0-4]\\d|25[0-5])\\/)|'  #IPv4
    ...
    def web_traffic(self, url):
        '''check the rank of url, if the rank is low, it's probably phishing websites'''
        try:
    ...
    def domain_registration_length(self, url):
        '''
        check the registration length of domain (end_date - today), if the url's registration length
        is short(< 1 year), it's probably phishing websites.
        '''
        dns = 0
        try:
    ...
    def age_domain(self, url):
        ''' age of the domain, if < half year, it's probably phishing website '''
        dns = 0
        try:
            domain_name = whois.whois(urlparse(url).netloc)
        except:
    ...
    def dns_record(self, url):
        ''' look up domain name by WHOIS database '''
        dns = 0
        try:
            domain_name = whois.whois(urlparse(url).netloc)
        except:
    ...
    def statistical_report(self, url):
        '''check some speical domain or ip address'''
        hostname = url
        ...
        url_match = re.search('at\.ua|usa\.cc|baltazarpresentes\.com\.br|pe\.hu|esy\.es|hol\.es|'
                              'sweddy\.com|myjino\.ru|96\.lt|ow\.ly', url)
        try:
            ip_address = socket.gethostbyname(hostname)
            ip_match = re.search('146\.112\.61\.108|213\.174\.157\.151|121\.50\.168\.88|192\.185\.217\.116|'
                                 ...
                                 '119\.28\.52\.61|54\.83\.43\.69|52\.69\.166\.231|216\.58\.192\.225|118\.184\.25\.86|'
                                 '67\.208\.74\.71|23\.253\.126\.58|104\.239\.157\.210|175\.126\.123\.219|141\.8\.224\.221|'
                                 '10\.10\.10\.10|43\.229\.108\.32|103\.232\.215\.140|69\.172\.201\.153|216\.218\.185\.162|'
                                 '54\.225\.104\.146|103\.243\.24\.98|199\.59\.243\.120|31\.170\.160\.61|213\.19\.128\.77|'
                                 '62\.113\.226\.131|208\.100\.26\.234|195\.16\.127\.102|195\.16\.127\.157|34\.196\.13\.28|'
                                 '103\.224\.212\.222|172\.217\.4\.225|54\.72\.9\.51|192\.64\.147\.141|198\.200\.56\.183|'
    ...
    def https_token(self, url):
        '''check whether have https token'''
...
def getAttributess(url):
    ''' Object creation and put it into dataframe '''
    ...
    d = {'Having_@_symbol': pd.Series(having_at_symbol), 'Having_IP': pd.Series(having_ip),
         'Prefix_suffix_separation': pd.Series(prefix_suffix_separation), 'Redirection_//_symbol': pd.Series(redirection_symbol),
         ...
         'http_tokens': pd.Series(http_tokens), 'statistical_report': pd.Series(statistical_report),
